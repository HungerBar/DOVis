...
# -----------------------------
def build_cache_key(
    time_index: int, threshold: float, depth_index: int, model_version: str = "v1"
) -> str:
    """
    生成唯一 cache key
    """
...
BASE_CACHE_DIR = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "..", "..", "tiles", "hypoxia")
)
...
def get_cache_dir(cache_key: str) -> str:
    ...
    return os.path.join(BASE_CACHE_DIR, cache_key)
