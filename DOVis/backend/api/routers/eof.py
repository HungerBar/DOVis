...
def run_eof(req: EOFRequest):
    ...
    if not (0 <= req.time_range[0] < ds.dims["time"]) or not (0 <= req.time_range[1] < ds.dims["time"]):
    ...
    if req.mode_type == "horizontal":
        ...
        if not (0 <= depth_idx < ds.dims["depth"]):
            raise HTTPException(status_code=400, detail=f"深度层级超出范围，应在 0-{ds.dims['depth']-1} 层之间。")
    ...
    # 3. 剖面切片模式：对输入的经纬度空间边界进行刚性防御
        ...
        if sparams["type"] not in ["lat", "lon"]:
            raise HTTPException(status_code=400, detail="剖面类型必须为 'lat' 或 'lon'。")
            
        val = float(sparams["value"])
        if sparams["type"] == "lat":
            lat_min, lat_max = float(ds["lat"].values.min()), float(ds["lat"].values.max())
            if not (lat_min <= val <= lat_max):
                raise HTTPException(status_code=400, detail=f"纬度剖面位置越界！当前数据集支持范围: {lat_min} 到 {lat_max}")
        else:
            lon_min, lon_max = float(ds["lon"].values.min()), float(ds["lon"].values.max())
            if not (lon_min <= val <= lon_max):
                raise HTTPException(status_code=400, detail=f"经度剖面位置越界！当前数据集支持范围: {lon_min} 到 {lon_max}")
